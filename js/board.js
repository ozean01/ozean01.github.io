/* ============ 🗂 本周学习任务看板（软包装外贸 · TaskBoard） ============
   ...
   ⚠️ 可达性现状（R5 收尾时登记，见方案 §6.2）：`#/board` 已不是独立页面——app.js 的
   REDIRECT_ROUTES 把它整页重定向到 `#/today`，今日页内只用本模块的
   `weekSummaryHtml()` 渲染只读「本周」区（today.js）。所以下面的 `render()` 三列看板
   目前**没有可达入口**（保留实现，未删除；若将来要复活，需先解决"全站计划源只有一处"
   的门禁，见 tools/test-home.js 的防复活断言）。计划源已统一到「今日」。
